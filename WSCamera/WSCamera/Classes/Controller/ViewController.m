...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *touchBtnLeftConstraint;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *touchBtnBottomConstaint;
...
- (void)viewDidLoad {
    ...
    self.titleView.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"seacher_bg_image"]];
    self.webView.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"seacher_bg_image"]];
    NSURL *url = [NSURL URLWithString:@"https://www.baidu.com"];
    NSURLRequest *request = [NSURLRequest  requestWithURL:url];
    [self.webView loadRequest:request];

    UIPanGestureRecognizer *panGesture = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(panView:)];
    [self.touchBtn addGestureRecognizer:panGesture];
...
{
    WSSettingViewController *vc = [CPMainStoryBoard instantiateViewControllerWithIdentifier:@"WSSettingViewController"];
...
- (void)panView:(UIPanGestureRecognizer *)panGesture
{
    ...
    if (panGesture.state == UIGestureRecognizerStateChanged)
    {
        ...
        CGFloat iconX = self.touchBtnLeftConstraint.constant + movePoint.x;
        CGFloat iconY = self.touchBtnBottomConstaint.constant- movePoint.y;
        if (iconX > 0&&iconX + 60 < CPScreenWidth)
        {
            self.touchBtnLeftConstraint.constant += movePoint.x;
        }
        if (iconY > 0 && iconY + 60 < CPScreenHeight) {
            self.touchBtnBottomConstaint.constant -= movePoint.y;
        }
    ...
    {
        if (CPScreenHeight - self.touchBtn.y > 200)
        {
            if (self.touchBtn.x <= CPScreenWidth / 2)
            {
                self.touchBtnLeftConstraint.constant = 0;
            }
            if (self.touchBtn.x > CPScreenWidth / 2) {
                self.touchBtnLeftConstraint.constant = CPScreenWidth - self.touchBtn.width;
            }
        }
        else
        {
            self.touchBtnBottomConstaint.constant  = 0;
        }
    }
