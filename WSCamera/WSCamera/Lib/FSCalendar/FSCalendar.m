...
- (void)initialize
{
    ...
    _weekdayFont      = [UIFont systemFontOfSize:17];
    _headerTitleFont  = [UIFont systemFontOfSize:18];
    _headerTitleColor = CPRGBColor(128, 128, 128);
    ...
    _backgroundColors[@(FSCalendarCellStateNormal)]      = CPRGBColor(255, 255, 255);
    _backgroundColors[@(FSCalendarCellStateSelected)]    = CPRGBColor(22, 168, 233);
    _backgroundColors[@(FSCalendarCellStateDisabled)]    = CPRGBColor(246, 246, 244);
    _backgroundColors[@(FSCalendarCellStatePlaceholder)] = CPRGBColor(246, 246, 244);
    _backgroundColors[@(FSCalendarCellStateToday)]       = CPRGBColor(234, 234, 234);

    _titleColors = [NSMutableDictionary dictionaryWithCapacity:4];
    _titleColors[@(FSCalendarCellStateNormal)]      = CPRGBColor(159, 159, 159);
    _titleColors[@(FSCalendarCellStateSelected)]    = [UIColor whiteColor];
    _titleColors[@(FSCalendarCellStateDisabled)]    = CPRGBColor(145, 145, 145);
    _titleColors[@(FSCalendarCellStatePlaceholder)] = CPRGBColor(140, 140, 140);
...
//    _eventColor = [kBlue colorWithAlphaComponent:0.75];
    _eventColor = CPRGBColor(253, 144, 22);
    _cellStyle = FSCalendarCellStyleCircle;
    _autoAdjustTitleSize = YES;
    
    _topBorderLayer = [CALayer layer];
    _topBorderLayer.backgroundColor = [[UIColor grayColor] colorWithAlphaComponent:0.2].CGColor;
...
- (void)loadWeekDays {
    ...
    NSArray *weekdayEng = @[@"Sun", @"Mon", @"Tue", @"Wed", @"Thu", @"Fri", @"Sat"];
    NSArray *weekdayGB = @[@"日", @"一", @"二", @"三", @"四", @"五", @"六"];
    
    NSArray *weekSymbols = nil;
    if ([[InternationalControl userLanguage] isEqualToString:@"en"]) {
        weekSymbols = weekdayEng;
    } else {
        weekSymbols = weekdayGB;
    }
    ...
    {
        UILabel *weekdayLabel = (UILabel *)[self viewWithTag:(TagWeekDayLabel + i)];
        if (weekdayLabel)
        {
...
//            DLog(@"%@", weekSymbols[i]);
            ...
            if (i == 0 || i == 6)
            {
                weekdayLabel.textColor = CPRGBColor(251, 125, 0);
            }
            else
            {
                weekdayLabel.textColor = CPRGBColor(116, 116, 116);
            }
            
            weekdayLabel.tag = TagWeekDayLabel + i;
